...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_recipe'])) {
    ...
    try {
        ...
        // Insert updated ingredients and their alternative options
        $insertIngredientsStmt = $pdo->prepare("INSERT INTO ingredients (meal_id, ingredient_name, alt_ingredients) VALUES (?, ?, ?)");
        $ingredientsArray = explode("\n", $all_ingredients);
        $altIngredientsArray = explode("\n", $alt_ingredients);

        foreach ($ingredientsArray as $index => $ingredient) {
            $altIngredient = $altIngredientsArray[$index] ?? null; // Use null if no alternative is provided
            $insertIngredientsStmt->execute([$meal_id, trim($ingredient), trim($altIngredient)]);
        }
